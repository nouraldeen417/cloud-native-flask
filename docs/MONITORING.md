...
### Query 3: Ingress 5xx error rate

```kql
ContainerLog
| where TimeGenerated > ago(24h)
| where ContainerID has "ingress-nginx"
| where LogEntry matches regex @'" (5\d{2}) '
| extend StatusCode = extract(@'" (5\d{2}) ', 1, LogEntry)
| summarize ErrorCount = count() by StatusCode, bin(TimeGenerated, 15m)
| order by TimeGenerated desc
```

> If this comes back empty, check the actual ingress-nginx log format first:
> ```bash
...
**After each query runs successfully:** click **Save** (top toolbar) → give it a clear name (e.g. `failed-container-starts`, `high-resource-usage`, `ingress-5xx-rate`) → this makes it reusable and pinnable in the next steps.

---

## Step 3 — Create an alert rule

Using the memory query as the trigger condition (adjust threshold to match your pod's `resources.limits.memory: 256Mi`):
...
3. Repeat for all three saved queries
4. Arrange tiles as preferred in the dashboard editor
